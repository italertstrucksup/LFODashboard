...
{
    private readonly ProfileDetailsDL _dl;

    public ProfileDetailsBL(ProfileDetailsDL dl)
    {
        _dl = dl;
    }
    ...
    #region Upsert
    public async Task<ProfileResponse> UpsertProfileAsync(UpsertProfileRequest request)
    {
        var success = await _dl.UpsertProfileAsync(request);

        if (!success)
            throw new Exception("Failed to save profile");

        // 🔥 Fetch updated data (best practice)
        var profile = await GetProfileDetailsByIdAsync((int)request.LoginId);
    ...
    {
        var result = await _dl.UpsertDocumentsAsync(request);
    ...
    public async Task<bool> AddLaneAsync(PreferredLaneRequest request)
    {
        var result = await _dl.InsertLaneAsync(request);
    ...
    public async Task<bool> AddTruckAsync(TruckDetailsRequest request)
    {
        var result = await _dl.InsertTruckAsync(request);
    ...
    {
        var result = await _dl.UpsertKYCAsync(request);
